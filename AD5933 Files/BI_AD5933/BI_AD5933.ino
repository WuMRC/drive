...
double SYSTEM_PHASE_SHIFT = 0;       // Initialize voltage phase shift value.
...
double* PS_Array = NULL; // Pointer for dynamic voltage phase shift array size.
...
void setup() {
  ...
  AD5933.setupDevicePins(HIGH); // Bi-polar mode
  ...
  AD5933.getGainFactor(CAL_RESISTANCE, CAL_SAMPLES, GAIN_FACTOR, SYSTEM_PHASE_SHIFT, false);
...
void loop() {
  ...
  if (SAMPLE_RATE_FLAG) { // this flag is toggled from the timer's interrupt context. It controls the sample rate. 
    ...
    if(!FREQ_SWEEP_FLAG) { // Repeat frequency, don't sweep.
      ...
      AD5933.getComplex(GAIN_FACTOR, SYSTEM_PHASE_SHIFT, z_mag, phaseAngle);
      ...
      AD5933.getComplex(GF_Array[currentStep], PS_Array[currentStep], z_mag, phaseAngle);
...
void adjustAD5933(int purpose, int v1, int v2, int v3) {
  ...
  case FR:
    ...
    if(stepSize == 0) { // frequency sweep is disabled
      ...
      AD5933.getComplex(GAIN_FACTOR, SYSTEM_PHASE_SHIFT, CR_Array[0], phaseAngle);
      ...
      Serial.print("\t");
      Serial.print(SYSTEM_PHASE_SHIFT, 4);
      Serial.print("\t");
      Serial.print("CR: ");
      Serial.print("\t");
      ...
      AD5933.getGainFactorsSweep(CAL_RESISTANCE, CAL_SAMPLES, GF_Array, PS_Array);
